...
program
  .option('--mode <mode>', 'Modo de trabajo', 'dev') // Por default ejecutamos en modo dev
program.parse()
...
//   const CONNECTION_MONGO = `mongodb://${config.db_host}/${config.db_name}`
//   connectionFn(CONNECTION_MONGO);
// } else {
  const config = configFn() 
  const CONNECTION_MONGO = `mongodb+srv://${config.db_user}:${config.db_password}@${config.db_host}/${config.db_name}?retryWrites=true&w=majority`
  connectionFn(CONNECTION_MONGO);
// }
